...
{	
	...
	/**
	 * 抓取新的小说，初始化抓取任务表和小说概览表， 写入两个表用事务来管理
	 * @throws IOException 
	 */
	@Transactional()
	public void CrawleNewNovel(String sourceUrl) throws IOException
	{
		String sHtml = HttpClientUtils.getContent(sourceUrl);
		if (sHtml == null) {
			logger.error("fetch {} error, not continue crawl",sourceUrl);
			return;
		}
		
		String charset =  RexxarPageUtils.getCharSetByBody(sHtml);
		//logger.debug("Response charset:  " +  charset);
		
		//httpclient default charset:  ISO-8859-1
